...
For constant $B_d$ and constant $w_{\rm th}$, the physical adiabatic background sound speed is
\begin{equation}
 c_a^2\equiv c^2\frac{\dot p_d}{\dot\rho_d}
 =w_{\rm th}c^2,
 \label{eq:hidden-soundspeed}
\end{equation}
where $p_d$ and $\rho_d$ are written in the same energy-density units used in $w_d=p_d/\rho_d$. Equivalently, in units $c=1$, $c_a^2=\dot p_d/\dot\rho_d=w_{\rm th}$. This relation assumes the vacuum offset does not fluctuate independently. Thus the background value $w_d\to-1$ does not force the perturbation sound speed to approach zero or acquire a negative squared value; perturbations must be specified separately.
