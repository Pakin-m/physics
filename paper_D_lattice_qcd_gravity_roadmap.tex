...
\begin{equation}
 W[g]\equiv-\ln Z[g].
\end{equation}
Consistently with the convention above for $\ln Z$, one has
\begin{equation}
 \langle T_{\mu\nu}(x)\rangle_g
 =-\frac{2}{\sqrt{g(x)}}\frac{\delta W}{\delta g^{\mu\nu}(x)},
 \label{eq:lqcd-onepoint}
\end{equation}
and it is convenient to define the matter polarization kernel with the corresponding sign,
\begin{equation}
 \Pi_{\mu\nu,\rho\sigma}(x,y)
 \equiv
...
\end{equation}
With this sign convention, and up to the contact terms generated by varying the stress tensor itself, $\Pi$ is the connected $TT$ response kernel. A small external metric perturbation obeys the linear-response relation
\begin{equation}
 \delta\langle T_{\mu\nu}(x)\rangle
