...
The unnormalized tensors above are useful overlap diagnostics. An exact source decomposition can be constructed when the $c_{ia}$ are interpreted as nonnegative coupling strengths. Define
\begin{equation}
 N_i=\left(\sum_a c_{ia}\right)^2
 =\sum_a c_{ia}^2+2\sum_{a<b}c_{ia}c_{ib},
\end{equation}
for every excitation with $N_i>0$, and define
\begin{align}
 \overline T^{(a)}_{\mu\nu}
 &=\sum_i\frac{c_{ia}^2}{N_i}T^{(i)}_{\mu\nu},\\
 \overline T^{(ab)}_{\mu\nu}
 &=\sum_i\frac{2c_{ia}c_{ib}}{N_i}T^{(i)}_{\mu\nu},
 \qquad a<b.
...
\begin{equation}
 \frac{\sum_a c_{ia}^2+2\sum_{a<b}c_{ia}c_{ib}}{N_i}=1.
\end{equation}
Multiplying by $T^{(i)}_{\mu\nu}$ and summing over $i$ gives Eq.~\eqref{eq:exact-overlap-reconstruction}.
\end{proof}

This construction preserves the original overlap idea while turning Eq.~\eqref{eq:decomp} into an exact partition rather than a schematic sum. Signed charges can still be retained in $\Omega_{ab}$ for interference-like diagnostics; the nonnegative strengths used in Eq.~\eqref{eq:exact-overlap-partition} serve only as partition weights.
